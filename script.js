...
function updateSelectedProductsDisplay() {
  if (selectedProducts.length === 0) {
    selectedProductsList.innerHTML =
      '<p class="no-products">No products selected yet. Click on product cards to add them!</p>';
    return;
  }
...
function addClearAllButtonListener() {
  ...
  if (clearAllBtn) {
    clearAllBtn.addEventListener("click", (e) => {
      ...
      /* Clear all products without confirmation */
...
function applyFiltersAndDisplay() {
  ...
  if (filteredProducts.length === 0) {
    ...
    if (searchTerm) {
      ...
      resultsInfo.innerHTML = `Found ${filteredProducts.length} product${
        filteredProducts.length !== 1 ? "s" : ""
      }${categoryText} matching "${searchTerm}"`;
...
async function sendMessageToAI(userMessage, isRoutineGeneration = false) {
  try {
    ...
    if (selectedProducts.length > 0) {
      selectedProductsContext = `\n\nCurrent selected products: ${selectedProducts
...
Provide helpful, friendly advice. Keep responses concise but informative.${selectedProductsContext}`,
    ...
    const recentHistory =
      conversationHistory.slice(-20); /* Last 10 user-AI exchanges */
...
document
  ...
  .addEventListener("click", async () => {
    ...
    addMessageToChat(
      `Generate a routine using my ${selectedProducts.length} selected products`,
      true
    );
